...
async function createAPI() { // Install funktion
  for (let i = 1; i < pokemonEnglish.length; i++) {
    console.clear()
    console.log((100 * i / pokemonEnglish.length).toFixed(2), " % : ", i, " / ", pokemonEnglish.length);
...
async function loadName(x) {
  const API = `https://pokeapi.co/api/v2/pokemon-species/${x}/`;
  let response = await fetch(API)
  let pokemon = await response.json()
  let name = pokemon['names']['5']['name'];
  return name;

}
async function loadAbility(x) {
  const API = `https://pokeapi.co/api/v2/ability/${x}/`;
  let response = await fetch(API)

  let abilityData = await response.json()
  let name = abilityData['names']['4']['name'];
  return name;

}
async function loadType(x) {
  const API = `https://pokeapi.co/api/v2/type/${x}/`;
  let response = await fetch(API)

  let typeData = await response.json()
  let name = typeData['names']['4']['name'];
  return name;

}
async function loadFlavorText(x) {
  const API = `https://pokeapi.co/api/v2/pokemon-species/${x}/`;
  let response = await fetch(API)
  let fTData = await response.json()
  let name = fTData['flavor_text_entries']['33']['flavor_text'] || fTData['flavor_text_entries']['88']['flavor_text'] || fTData['flavor_text_entries']['41']['flavor_text'];
  return name;
...
async function downloadPokemon(i) {
  ...
  let statsINIT = myPokemon['stats']['5']['base_stat'];
  let pkmnId = myPokemon['id']
  let pkmnName = await loadName(pkmnId)
  ...
  let pkmnType1Eng = myPokemon['types']['0']['type']['name'];
  let pkmnType1Ger = await loadType(pkmnType1Eng)

  let pkmnType2Eng = myPokemon['types'][1] && myPokemon['types'][1]['type']['name'];
  let pkmnType2 = (pkmnType2Eng) ? await loadType(pkmnType2Eng) : undefined;
    let moves = async () => {
    allMoves = [];
    for (let i = 0; i < 4; i++) {
      if (myPokemon['abilities'][i]) {
        let abiltiyEng = myPokemon['abilities'][i]['ability']['name']
        let abiltiyGer = await loadAbility(abiltiyEng);
        allMoves.push(abiltiyGer);
      }
    }
    return allMoves;
  }
  let cry = myPokemon['cries']['latest']
  let fT = async()=>{
    let flavorT;
    try{
      flavorT = await loadFlavorText(pkmnId)
    }
    catch(e){
      console.error(e)
    }
  ...
  const newPokemon = new Pokemon(pkmnName, pkmnId, frontHome, frontClassic, pkmnType1Ger, pkmnType2, await moves(), statsHP, statsATK, statsDEF, statsINIT, statsSATK, statsSDEF, cry, await fT())
